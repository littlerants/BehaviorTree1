...
import carla
from carla_api.agents.navigation.controller import VehiclePIDController
from carla_api.agents.tools.misc import draw_waypoints, get_speed, cal_distance_angle
    ...
    def __init__(self, vehicle, opt_dict={}, map_inst=None):
        ...
        if map_inst:
            if isinstance(map_inst, carla.Map):
                ...
                print(
                    "Warning: Ignoring the given map as it is not a 'carla.Map'"
                )
        ...
        if opt_dict:
            ...
            if "desired_velocity" in opt_dict:
                self._target_speed = opt_dict["desired_velocity"]
            ...
            if "max_speed" in opt_dict:
                self._max_speed = opt_dict["max_speed"]
    ...
    def _compute_next_waypoints(self, k=1):
        ...
        for _ in range(k):
            if len(self._waypoints_queue) > 0:
                ...
                last_waypoint = self._map.get_waypoint(self._vehicle.get_location())
    ...
    def run_step(self, debug=False):
        ...
        for waypoint, _ in self._waypoints_queue:          
            _, angle = cal_distance_angle(
                waypoint.transform.location, veh_location, veh_yaw
            )
            if angle > 90:
                num_way_removed += 1
            else:
        ...
        if num_way_removed > 0:
            for _ in range(num_way_removed):
                self._waypoints_queue.popleft()
