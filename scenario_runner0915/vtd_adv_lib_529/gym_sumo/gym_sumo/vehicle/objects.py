...
if TYPE_CHECKING:
    ...
    from gym_sumo.road.road import Road
...
class RoadObject(ABC):
    ...
    def __init__(self, road: 'Road', name: str, position: Sequence[float], heading: float = 0, speed: float = 0, lane_index: int = None,):
        """
        ...
        if name.startswith('ped'):
            ...
            self.lane_index = self.road.network.get_closest_lane_index(self.position, self.heading, lane_index=lane_index) if self.road else np.nan
    ...
    @classmethod
    def make_on_lane(cls, road: 'Road', name: str, lane_index: LaneIndex, longitudinal: float, speed: Optional[float] = None) \
            -> 'RoadObject':
        """
        ...
        return cls(road, name, lane.position(longitudinal, 0), lane.heading_at(longitudinal), speed, lane_index=lane_index)
    ...
    def on_road(self) -> bool:
        ...
        left_change_lane, right_change_lane = self.road.network.can_change_lane(self.lane_index)

        if left_change_lane and right_change_lane:
            return True

        return self.lane.on_lane(self.position, have_right_lane=right_change_lane, have_left_lane=left_change_lane)
