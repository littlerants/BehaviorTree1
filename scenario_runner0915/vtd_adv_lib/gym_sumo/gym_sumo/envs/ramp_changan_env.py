...
class RampEnv(AbstractEnv):
    ...
    def default_config(cls) -> dict:
        ...
        cfg.update({
            ...
            'make_road': False,
            'map_file_path': ['/home/sx/wjc/wjc/datasets/ca_vtd_train_data/zadao/1027_01.xodr',
                              '/home/sx/wjc/wjc/datasets/ca_vtd_train_data/zadao/0228.xodr'],
    ...
    def _reset(self) -> None:
        ...
        if self.config['make_road']:
            self.name = 'ego'
            self.map_file_path = random.choice(self.config['map_file_path'])
            self.map = GlobalRoutePlanner(self.map_file_path)
            self._make_road()
            self._make_vehicles()
            self.pre_data = dict()
        else:
    ...
    def _make_vehicles(self) -> None:
        ...
        if self.map_file_path.endswith('1027_01.xodr'):
            scenarios = {
                'block_on_ramp': {'ego_road_info': (2, 0, -4), 'test_av_road_info': (2, 0, -3), 'ego_obj_road_info': (7, 0, -3), 'test_av_obj_road_info': (7, 0, -3),},
                'block_on_ramp1': {'ego_road_info': (2, 0, -3), 'test_av_road_info': (2, 0, -2), 'ego_obj_road_info': (7, 0, -3), 'test_av_obj_road_info': (7, 0, -3),},
                'block_on_ramp2': {'ego_road_info': (2, 0, -2), 'test_av_road_info': (2, 0, -1), 'ego_obj_road_info': (7, 0, -3), 'test_av_obj_road_info': (7, 0, -3),},
                'block_off_ramp_1': {'ego_road_info': (6, 0, -3), 'test_av_road_info': (6, 0, -4), 'ego_obj_road_info': (7, 0, -3), 'test_av_obj_road_info': (7, 0, -3)},
                'block_off_ramp_2': {'ego_road_info': (6, 0, -3), 'test_av_road_info': (6, 0, -2), 'ego_obj_road_info': (7, 0, -3), 'test_av_obj_road_info': (7, 0, -3)},
                'block_off_ramp_3': {'ego_road_info': (6, 0, -2), 'test_av_road_info': (6, 0, -1), 'ego_obj_road_info': (7, 0, -3), 'test_av_obj_road_info': (7, 0, -3)},
                'lane_reduction': {'ego_road_info': (4, 1, -1), 'test_av_road_info': (4, 1, -2), 'ego_obj_road_info': (7, 0, -3), 'test_av_obj_road_info': (7, 0, -3)},
                'block_on_ramp_same_lane': {'ego_road_info': (2, 0, -3), 'test_av_road_info': (2, 0, -3), 'ego_obj_road_info': (7, 0, -3), 'test_av_obj_road_info': (7, 0, -3)},
                'block_on_ramp1_same_lane': {'ego_road_info': (2, 0, -2), 'test_av_road_info': (2, 0, -2), 'ego_obj_road_info': (7, 0, -3), 'test_av_obj_road_info': (7, 0, -3)},
                'block_on_ramp2_same_lane': {'ego_road_info': (2, 0, -1), 'test_av_road_info': (2, 0, -1), 'ego_obj_road_info': (7, 0, -3), 'test_av_obj_road_info': (7, 0, -3)},
                'block_off_ramp1_same_lane': {'ego_road_info': (6, 0, -4), 'test_av_road_info': (6, 0, -4), 'ego_obj_road_info': (7, 0, -3), 'test_av_obj_road_info': (7, 0, -3)},
                'block_off_ramp2_same_lane': {'ego_road_info': (6, 0, -3), 'test_av_road_info': (6, 0, -3), 'ego_obj_road_info': (7, 0, -3), 'test_av_obj_road_info': (7, 0, -3)},
                'block_off_ramp3_same_lane': {'ego_road_info': (6, 0, -2), 'test_av_road_info': (6, 0, -2), 'ego_obj_road_info': (7, 0, -3), 'test_av_obj_road_info': (7, 0, -3)},
                'block_off_ramp4_same_lane': {'ego_road_info': (6, 0, -1), 'test_av_road_info': (6, 0, -1), 'ego_obj_road_info': (7, 0, -3), 'test_av_obj_road_info': (7, 0, -3)},
                'lane_reduction_same_lane': {'ego_road_info': (4, 1, -2), 'test_av_road_info': (4, 1, -2), 'ego_obj_road_info': (7, 0, -3), 'test_av_obj_road_info': (7, 0, -3)},
            }
        elif self.map_file_path.endswith('0228.xodr'):
            if self.config['AtomicScene'] in ['lon_mode']:
                road_infos = [(131, 34), (139, 42), (138, 138)]
                scenarios = {
                    'scene_1': {
                        'ego_road_info': (road_infos[0][0], 0, -3),
                        'test_av_road_info': (road_infos[0][0], 0, -2),
                        'ego_obj_road_info': (road_infos[0][1], 0, -1),
                        'test_av_obj_road_info': (road_infos[0][1], 0, -1)
                    },
                    'scene_2': {
                        'ego_road_info': (road_infos[0][0], 0, -2),
                        'test_av_road_info': (road_infos[0][0], 0, -1),
                        'ego_obj_road_info': (road_infos[0][1], 0, -1),
                        'test_av_obj_road_info': (road_infos[0][1], 0, -1)
                    },
                    'scene_3': {
                        'ego_road_info': (road_infos[1][0], 0, -1),
                        'test_av_road_info': (road_infos[1][0], 0, -2),
                        'ego_obj_road_info': (road_infos[1][1], 0, -1),
                        'test_av_obj_road_info': (road_infos[1][1], 0, -1)
                    },
                    'scene_4': {
                        'ego_road_info': (road_infos[1][0], 0, -2),
                        'test_av_road_info': (road_infos[1][0], 0, -1),
                        'ego_obj_road_info': (road_infos[1][1], 0, -1),
                        'test_av_obj_road_info': (road_infos[1][1], 0, -1)
                    },
                    'scene_5': {
                        'ego_road_info': (road_infos[2][0], 0, -1),
                        'test_av_road_info': (road_infos[2][0], 0, -2),
                        'ego_obj_road_info': (road_infos[2][1], 0, -1),
                        'test_av_obj_road_info': (road_infos[2][1], 0, -1)
                    },
                    'scene_6': {
                        'ego_road_info': (road_infos[2][0], 0, -2),
                        'test_av_road_info': (road_infos[2][0], 0, -1),
                        'ego_obj_road_info': (road_infos[2][1], 0, -1),
                        'test_av_obj_road_info': (road_infos[2][1], 0, -1)
                    },
                }
            elif self.config['AtomicScene'] in ['lat_mode']:
                road_infos = [(131, 34), (139, 42), (138, 138)]
                scenarios = {
                    'scene_1': {
                        'ego_road_info': (road_infos[0][0], 0, -3),
                    ...
                    'scene_3': {
                        'ego_road_info': (road_infos[0][0], 0, -3),
                        'test_av_road_info': (road_infos[0][0], 0, -3),
                        'ego_obj_road_info': (road_infos[0][1], 0, -1),
                        'test_av_obj_road_info': (road_infos[0][1], 0, -1)
                    },
                    'scene_4': {
                    ...
                    'scene_12': {
                        'ego_road_info': (road_infos[2][0], 0, -2),
                        'test_av_road_info': (road_infos[2][0], 0, -2),
                        'ego_obj_road_info': (road_infos[2][1], 0, -2),
                        'test_av_obj_road_info': (road_infos[2][1], 0, -2)
                    },
                }

            else:
                raise ValueError('map_file_path is not supported')  # 如果map_file_path不支持，则抛出ValueError异常
        ...
        for frame in range(frames):
            ...
            if self.config['action']['type'] == 'ContinuousAction':  # 如果动作类型是连续的
                ...
                if self.config['AtomicScene'] == 'lat_mode':
                    self.vehicle.speed = self.ego_vehicle.speed
