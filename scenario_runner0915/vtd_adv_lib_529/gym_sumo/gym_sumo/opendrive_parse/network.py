...
from gym_sumo.opendrive_parse.utils import encode_road_section_lane_width_id
from gym_sumo.opendrive_parse.converter import OpenDriveConverter
from gym_sumo.opendrive_parse.elements.opendrive import OpenDrive
...
class Network:
    ...
    def load_opendrive(self, opendrive: OpenDrive):
        ...
        for road in opendrive.roads:
            precision = 1 if road.planView.length > 1 else 0.5
            road.planView.precalculate(precision)
            start_pre_precalculation_idx = 0
            start_long_off = 0.0
            for lanesection in road.lanes.lane_sections:

                lanesection.process(road.planView.get_precalculation, start_pre_precalculation_idx, precision, start_long_off)
                start_pre_precalculation_idx = lanesection.end_pre_precalculation_idx+1
