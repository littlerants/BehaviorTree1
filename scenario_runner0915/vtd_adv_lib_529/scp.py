...
class SCP():
    ...
    def turn_right(self,actor,num = -1):
        data = "<Traffic><ActionLaneChange direction='{}' force='true' delayTime='0.0' approveByDriver='false' activateOnExit='false' driverApproveTime='0' actor='{}' time='{}'/></Traffic>"
        data = data.format(num,actor,self.time)
        handle = self.get_handle(data)
        self.send(handle)
    # 这两左变道指令
...
# <Traffic><ActionLaneChange direction='1' force='true' delayTime='0.0' approveByDriver='false' activateOnExit='false' driverApproveTime='0' actor='Ego' time='3.0'/></Traffic>
# <Player name='Ego'><LaneOffset absolute='0' time='0' s='0'/></Player>