...
class HANDLE():
    ...
    def show(self):
        return ("(magicNo:{},version:{},headerSize: {},dataSize: {},frameNo: {},simTime: {})".format(self.magicNo,
                                                                                                     self.version,
                                                                                                     self.headerSize,
                                                                                                     self.dataSize,
                                                                                                     self.frameNo,
...
class ADV_Manager:
    ...
    def __init__(self, open_vtd=False, vtdPort=48190, sensorPort=48195, plugins_path='data/lib/sendTrigger.so',
        ...
        if self.open_vtd:
            ...
            self.lib.addPkg.argtypes = [ctypes.c_double, ctypes.c_uint32, ctypes.c_double, ctypes.c_double,
            ...
            for edge_id in self.map.graph.edges:
                ...
                if edge['entry_waypoint'].junction_id is not None:
                    ...
                    self.junction_road_info[edge_id] = (
                        edge['entry_waypoint'].road_id, edge['entry_waypoint'].section_id,
    ...
    def create_objs(self):
        ...
        if self.gl.ego is not None:
            ...
            self.gl.ego.update(pos_x=self.ego_vehicle.get_location().x, pos_y=self.ego_vehicle.get_location().y,
                               ...
                               leftLaneId=ego_wp.get_left_lane().lane_id if ego_wp.get_left_lane() else None,
                               rightLaneId=ego_wp.get_right_lane().lane_id if ego_wp.get_right_lane() else None, \
        ...
        for i in objects:
            ...
            if i.id in self.gl.objects_set:
                for j in self.gl.objects:
                    if i.id == j.id:
                        j.update(pos_x=i.get_location().x, pos_y=i.get_location().y,
                ...
                obj = OBJECT(id=i.id, pos_x=i.get_location().x, pos_y=i.get_location().y, off_x=0,
                             pos_h=i.get_transform().rotation.yaw, hdg=i_wp.transform.rotation.yaw, \
                             vx=i.get_velocity().x, vy=i.get_velocity().y,
                             acc_x=i.get_acceleration().x, l=i.bounding_box.extent.x, w=i.bounding_box.extent.y,
                             acc_y=i.get_acceleration().y, obj_type=i.type_id, \
    ...
    def handleRDBitemObjects(self, simFrame, simTime, dataPtr, flag, data):
        ...
        if item_base[2] >= 1 and item_base[2] <= 5:
            ...
            result1 = {"id": result[0], "x": result[1], "y": result[2], "h": result[3], "vx": result[4], "vy": result[5]
                , "vh": result[6], "l": result[7], "w": result[8], "type": result[9], "acc_x": result[10],
                       "acc_y": result[11], "off_x": result[12], "name": result[-1]
                       }
    ...
    def handleRDBitemRoadPos(self, simFrame, simTime, dataPtr, flag, data):
        ...
        result1 = {"id": item_base[0], "roadId": item_base[1], "lane_id": item_base[2], "laneoffset": item_base[6],
                   "hdg": item_base[7]}
    ...
    def handleRDBitemLaneInfo(self, simFrame, simTime, dataPtr, flag, data):
        ...
        result1 = {"id": item_base[-2], "road_lane_id": item_base[1], "lanewidth": item_base[9],
                   "leftLaneId": item_base[3], "rightLaneId": item_base[4]}
                ...
                for i in light_stat:
                    ...
                    if temp < 0:
                        if self.gl.close_light:
                            result1 = {"id": 1, "ctrlId": item_base1[0], "light_cur_state": light_cur_state,
                                       "light_state": LIGHT_SIGNAL[i['type']]}
    ...
    def faster_act(self, var=1):
        ...
        #     acceleration = 0.5
        # 计算辆车安全距离
        self.safe_dis = (self.gl.adv.vx + self.gl.ego.vx) * 0.1 + 4 + math.pow(self.gl.ego.vx, 2) / (
                    2 * (-self.get_max_acc(adv_speed) - 0.03 * ((self.gl.adv.vx + self.gl.ego.vx) / 10))) + abs(
            self.gl.adv.vx + self.gl.ego.vx)
        ...
        if self.dis < ((self.safe_dis + 5) if self.safe_dis < 5 else 11):
            acceleration = 1.5 + abs(self.gl.ego.vx) if abs(self.gl.ego.vx) > 1 and abs(
                self.gl.ego.vx) < 3 else 1 - self.dis / (self.safe_dis + 5)
    ...
    def contrl_adv(self, lat_warn):
        ...
        #     return -1
            ...
            if self.action_marking in ['LANE_RIGHT',
                                       'LANE_LEFT'] and self.gl.adv.lane_id == self.gl.pre_adv_lane_id and self.keep_time < 300:
                self.keep_time += 10
            else:
                self.stop_lat_act()
            return -1
    ...
    def get_dqn_state(self, left_lane=1, right_lane=1):
        ...
        if self.in_juction and self.gl.ego.lane is not None and self.gl.adv.lane is not None:
            ...
            intersection, intersection_center = check_lane_intersection(ego_center, ego_lane_width, bv_center,
                                                                        bv_lane_width)
            ...
            if intersection and self.gl.adv.roadId != self.gl.ego.roadId:
                ...
                ego_current_lon, _ = self.gl.ego.lane.local_coordinates(
                    np.array([self.gl.ego.pos_x, self.gl.ego.pos_y]))
        ...
        if self.gl.adv.obj_type == 5:
            ...
            if len(self.gl.adv.pos_trajx) >= 25:
                ...
                if self.gl.adv.pos_trajy[-1] > -10 and self.gl.adv.pos_trajx[-1] > 0 and adv_p[-1] <= 15 and adv_p[
                    -1] > 0:
            ...
            # 直接根据主车车辆坐标系 x与y来进行确认交互点
                ...
                print("ADV---egospeed, advspeed, pos:", ego_speed, adv_speed, self.gl.adv.pos_trajx[-1],
                      self.gl.adv.pos_trajy[-1])
        ...
        return torch.tensor(traj_obs).unsqueeze(0), torch.tensor(ita_obs).unsqueeze(0), torch.tensor(ped_obs).unsqueeze(
            0)
    ...
    def collision_warning(self):
        ...
        if self.gl.front_vec_to_compete is not None:
            ...
            if self.gl.front_vec_to_compete.vx < 0:
                ttc = (self.get_speed(self.gl.front_vec_to_compete.pos_x,
                                      self.gl.front_vec_to_compete.pos_y) - 5) / self.get_speed(
                    self.gl.front_vec_to_compete.vx, self.gl.front_vec_to_compete.vy)
                safe_distance2ego = self.get_sqrt(self.gl.front_vec_to_compete.pos_x,
                                                  self.gl.front_vec_to_compete.pos_y) - 5.5
        ...
        #             lat_warn =  True
        adv_speed = self.get_speed(self.gl.adv.vx, self.gl.adv.vy)
        safe_dis = (self.gl.adv.vx + self.gl.ego.vx) * 0.1 + 4 + math.pow(self.gl.ego.vx, 2) / (
                    2 * (-self.get_max_acc(adv_speed) - 0.03 * ((self.gl.adv.vx + self.gl.ego.vx) / 10)))

        if self.ctrl_signal['lat'] == "LANE_LEFT":
            lat_request = self.lat_check.mobil(self.gl.adv, self.gl.left_neib_front_vec_to_compete,
                                               self.gl.left_neib_bake_vec_to_compete,
                                               None, None, direction='LEFT')
        elif self.ctrl_signal['lat'] == "LANE_RIGHT":
            lat_request = self.lat_check.mobil(self.gl.adv, self.gl.right_neib_front_vec_to_compete,
    ...
    def collision_check(self, vec0, vec1):
        ...
        v1_p.append(
            [rotate_operate(- vec1.l * 0.5, - vec1.w * 0.5, vec1.pos_h)[i] + center[i] for i in range(len(center))])
        v1_p.append(
            [rotate_operate(- vec1.l * 0.5, vec1.w * 0.5, vec1.pos_h)[i] + center[i] for i in range(len(center))])
    ...
    def get_time(self, obj):
        time = -1
        s = self.get_sqrt(self.prepare.confrontation_position[0] - obj.pos_x,
                          self.prepare.confrontation_position[1] - obj.pos_y)
