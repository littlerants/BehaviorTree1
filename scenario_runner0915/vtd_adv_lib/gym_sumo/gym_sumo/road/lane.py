...
from gym_sumo.road.spline import LinearSpline2D
from gym_sumo.utils import wrap_to_pi, Vector, get_class_path, class_from_path
...
class AbstractLane(object):
    ...
    def on_lane(self, position: np.ndarray, longitudinal: float = None, lateral: float = None, margin: float = 0,
                have_left_lane: bool = False, have_right_lane: bool = False) -> bool:
        """
        ...
        if have_left_lane and have_right_lane:
            return True

        if longitudinal is None or lateral is None:
            longitudinal, lateral = self.local_coordinates(position)

        half_width = self.width_at(longitudinal) / 2

        if have_left_lane:
            return -(half_width + margin) <= lateral

        if have_right_lane:
            return lateral <= half_width + margin

        # is_on = np.abs(lateral) <= half_width + margin and \
        #     -self.VEHICLE_LENGTH <= longitudinal < self.length + self.VEHICLE_LENGTH
        return np.abs(lateral) <= half_width + margin
    ...
    def after_end(self, position: np.ndarray, longitudinal: float = None, lateral: float = None) -> bool:
        ...
        return longitudinal > self.length - self.VEHICLE_LENGTH
...
class PolyLaneFixedWidth(AbstractLane):
    ...
    def __init__(
        ...
        self.priority = priority
        self.map_vector = []
        self.init_map_vector()
        self.lane_index = lane_index

    def init_map_vector(self):
        for i in range(10):
            log = (self.length / 10) * i
            p = self.position(log, 0).tolist()
            w = self.width
            heading = self.heading_at(log)
            dis_proportion = log / self.length
            self.map_vector.append(
                [p[0], p[1], w, heading, dis_proportion]
            )
...
class PolyLaneWithWidth(PolyLaneFixedWidth):
    """
