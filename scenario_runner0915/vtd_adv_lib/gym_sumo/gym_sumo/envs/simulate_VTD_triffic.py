...
#   #  print(listdp[0])
    ...
    def exec_lat_act(self):
        ...
        if self.get_speed( self.gl.adv.vx, self.gl.adv.vy) <  10:
            return
        gain = 0
...
class Model():
    ...
    def load_model(self, epoch, epoch_lat,epoch_ita , epoch_ped):
        ...
        if self.train:
            ...
            model_path_ped = os.path.join(self.output_path, f'{epoch_ped}_model_ckpt.pth' )
...
def make_args():
    ...
    if model == 1:
        ...
        args.add_argument('--load-model-id-ped', default=296, type=int) # 348   
    if model == 2:
        # # ramp 454  374      
        # # 3-1   497  341
        args.add_argument('--load-model-id', default=497, type=int) # 454  374
        args.add_argument('--load-model-id-lat', default=341, type=int) # exp4 400 407 exp5 417 455  370
    ...
    if model == 3:
        ...
        args.add_argument('--load-model-id', default=307, type=int) # 454  374
        args.add_argument('--load-model-id-lat', default=351, type=int) # exp4 400 407 exp5 417 455  370
        args.add_argument('--load-model-id-ita', default=359, type=int) # exp4 400 407 exp5 417 455  370
        args.add_argument('--load-model-id-ped', default=296, type=int) # 348   
    if model == 4:
        # lanechange
