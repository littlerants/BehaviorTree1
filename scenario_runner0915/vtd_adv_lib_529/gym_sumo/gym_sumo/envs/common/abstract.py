...
from gym_sumo.envs.common.action import action_factory, Action, DiscreteMetaAction, ActionType
from gym_sumo.envs.common.observation import observation_factory, ObservationType
from gym_sumo.envs.common.finite_mdp import finite_mdp
from gym_sumo.envs.common.graphics import EnvViewer
from gym_sumo.vehicle.behavior import IDMVehicle, LinearVehicle
...
class AbstractEnv(gym.Env):
    ...
    def __init__(self, config: dict = None, render_mode: Optional[str] = 'rgb_array') -> None:
        super().__init__()
    ...
    def ego_vehicle(self) -> Vehicle:
        ...
        return self.controlled_vehicles[-1]
        # return self.controlled_vehicles[1] if (self.controlled_vehicles and len(self.controlled_vehicles)>1) else None
    ...
    def default_config(cls) -> dict:
        ...
        return {
            ...
            "other_vehicles_type": "gym_sumo.vehicle.behavior.IDMVehicle",
            "screen_width": 600,  # [px]
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, bool, dict]:
        ...
        if self.vtd_manager.open_vtd:
            obs_flag = self._simulate_vtd(action)
        else:
            self._simulate(action)
        if obs_flag:
            obs = self.observation_type.observe(self.vtd_manager.open_vtd)
    ...
    def change_vehicles(self, vehicle_class_path: str) -> 'AbstractEnv':
        ...
        :param vehicle_class_path: The path of the class of behavior for other vehicles
                             Example: "gym_sumo.vehicle.behavior.IDMVehicle"
    ...
    def compute_ttc(self, front_veh, rear_veh, max_ttc=5.0):
        front_ttc = max_ttc
        av_lane = self.road.network.get_lane(rear_veh.lane_index)
        av_lon, _ = av_lane.local_coordinates(rear_veh.position)
        front_veh_lon, _ = av_lane.local_coordinates(front_veh.position)
        av_angle = av_lane.local_angle(rear_veh.heading, av_lon)
        front_veh_angle = av_lane.local_angle(front_veh.heading, front_veh_lon)

        bv_speed = front_veh.speed * np.cos(front_veh_angle)
        av_speed = rear_veh.speed * np.cos(av_angle)

        if bv_speed < av_speed:
            front_ttc = min(front_ttc, max(front_veh_lon - av_lon - 5, 0) / utils.not_zero(av_speed - bv_speed))
        return front_ttc
