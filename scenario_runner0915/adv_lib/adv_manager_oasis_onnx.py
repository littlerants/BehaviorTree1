...
from  model_namager_onnx import ModelManager
...
class ADV_Manager(AtomicBehavior):
    ...
    def __init__(self, ego_actor=None, adv_tf_param=None,  name="ADV_Manager"):
    ...
    def create_objs(self):
        ...
        if len(self.static_objects) > 0:
            for i in self.static_objects:
                ...
                wp = w.transform.location + carla.Location(z=1)
                # 转换为左手坐标系进行显示
                # wp.y = -wp.y
                # print("static wall point:",wp.x,wp.y)
    ...
    def parseRDBMessage(self):
        ...
        if len(self.gl.objects) >0:
            ...
            for i in self.gl.objects:
                ...
                # 主车50米以内所有车辆
                if (i.name.find("static")== -1 or not i.static) and  self.get_sqrt(i.pos_x, i.pos_y) < 50  and i.pos_x > 0 and self.gl.ego.lane_id * i.lane_id > 0:
                    # 主车后30米以内所有车辆
                    # print("ADV---i.pos_x > 3")
                    # i.show()
    ...
    def get_dqn_state_new(self):
        ...
        dim = len(obs)
        obs = np.array(obs).astype(np.float32).reshape(1, dim)
    ...
    def sample_actions_new(self,state):
        if self.model_type == 0:
            ...
            if self.model_manager.model_config_list["model_wall"].depart:
                ...
                action = self.model_manager.model.model_list["model_wall"].run(None, {self.model_manager.model.model_list["model_wall"].get_inputs()[0].name: state } )
        ...
        # lon model
            ...
            if self.model_manager.model_config_list["model_lon"].depart:
                ...
                action = self.model_manager.model.model_list["model_lon"].run(None, {self.model_manager.model.model_list["model_lon"].get_inputs()[0].name: state } )
            ...
            if self.model_manager.model_config_list["model_no_wall"].depart:
                ...
                action = self.model_manager.model.model_list["model_no_wall"].run(None, {self.model_manager.model.model_list["model_no_wall"].get_inputs()[0].name: state } )
            ...
            if self.model_manager.model_config_list["model_wall"].depart:
                ...
                action = self.model_manager.model.model_list["model_dynamic_wall"].run(None, {self.model_manager.model.model_list["model_dynamic_wall"].get_inputs()[0].name: state } )
