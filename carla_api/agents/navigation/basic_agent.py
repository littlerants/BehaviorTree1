...
import carla
from carla_api.agents.navigation.global_route_planner import GlobalRoutePlanner
from carla_api.agents.navigation.local_planner import LocalPlanner, RoadOption
from carla_api.agents.tools.misc import (
    compute_distance,
    get_speed,
...
class BasicAgent(object):
    ...
    but ignores stop signs.
    It has several functions available to specify the route
    that the agent must follow,
    as well as to change its parameters in case a different
    driving mode is desired.
    """
    ...
    def __init__(
        ...
        Initialization the agent paramters, the local and the global planner.
            ...
            :param opt_dict: dictionary in case some of its parameters want
            to be changed.
                This also applies to parameters related to the LocalPlanner.
            :param map_inst: carla.Map instance to avoid the expensive call of
            getting it.
            :param grp_inst: GlobalRoutePlanner instance to avoid the expensive
            call of getting it.
        ...
        self._base_vehicle_threshold = 8.0  # meters
        self._speed_ratio = 1
        self._max_brake = 0.8
        self._offset = 0
        if opt_dict is None:
            opt_dict = {}
            print(
                "Warning: No parameters were passed to the agent. Using default values, opt_dict is None"
            )
        # Change parameters according to the dictionary
        if "desired_velocity" in opt_dict:
            self._target_speed = opt_dict["desired_velocity"] * 3.6
        else:
            self._target_speed = target_speed

        if "obey_traffic_lights" in opt_dict:
            self._ignore_traffic_lights = (
                False if opt_dict["obey_traffic_lights"] else True
            )

        if "obey_speed_limit" in opt_dict:
            self._speed_limit_flag = opt_dict["obey_speed_limit"]
        else:
            self._speed_limit_flag = False
        # self.follow_speed_limits(self._speed_limit_flag)

        if "identify_object" in opt_dict:
            self._ignore_vehicles = False if opt_dict["identify_object"] else True
    ...
    def add_emergency_stop(self, control):
        """
        Overwrites the throttle a brake values of a control to
        perform an emergency stop.
        The steering is kept the same to avoid going out of the lane when
        stopping during turns
    ...
    def _vehicle_obstacle_detected(
        ...
        for target_vehicle in vehicle_list:
            ...
            if target_transform.location.distance(ego_location) > max_distance:
                # print("dayu ............    {}    {}" .format(target_transform.location.distance(ego_location), max_distance))
                continue
            ...
            # Simplified approach, using only the plan waypoints (similar to TM)
                ...
                # print("target_rear_transform: {}   ego_front_transform  {}    max distance  {}    low angle {}    up {}    ".format(
                #     target_rear_transform.location, ego_front_transform.location, max_distance, low_angle_th, up_angle_th))
                if is_within_distance(
