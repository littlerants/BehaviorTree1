...
class OBJECT():
    def __init__(self,simTime = -1,simFrame = -1, name='',id=100,lane_id=0,pos_x=0,off_x=1.39,pos_y=0,pos_h=0,hdg=0,vx=0,vy=0,
                ...
                leftLaneId = -1,rightLaneId = -3,  distToJunc=10000000000.0,light_state='GO',roadId=None,adv_vec=False,lane = None,static=False,wp=None):
    ...
    def trans_cood2(self, base_obj, position_flag = False,velocity_flag = False,acc_flag = False ,rotate = -1):
        ...
        if position_flag:
            ...
            # if deta_heading >= 0:
            #     self.pos_h = deta_heading if abs(deta_heading) < np.pi else  deta_heading - 2 * np.pi
            # else:
            #     self.pos_h = deta_heading if abs(deta_heading) < np.pi else  deta_heading + 2 * np.pi
        if velocity_flag:
    ...
    def update(self,simTime=-1, simFrame=-1,lane_id=None,pos_x=None,off_x = None, pos_y=None,pos_h=None,hdg=None,vx=None,vy=None,
                ...
                leftLaneId = None,rightLaneId = None,  distToJunc=None,light_state=None,roadId=None,lane = None,static=None,wp = None):
        if simTime != -1:
            self.simTime = simTime
        if simFrame != -1:
            self.simFrame = simFrame
    ...
    def update1(self,obj):
        ...
        self.lane = obj.lane
        self.direction_to_ego = obj.direction_to_ego
    ...
    def show(self):
       print("simTime:",self.simTime,"simFrame:",self.simFrame,"name:",self.name,"id:",self.id,"x/y:",self.pos_x, self.pos_y,\
             "vx/vy:",self.vx,self.vy, "h:",self.pos_h ,"hdg:", self.hdg, "acc_x/y:",self.acc_x,self.acc_y,"roadId:",\
             self.roadId, "lane_id:",self.lane_id, "leftLaneId:",self.leftLaneId, "rightLaneId",self.rightLaneId )
    # 车辆坐标系转换为全局坐标系
