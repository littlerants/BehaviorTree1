...
class ADV_Manager:
    ...
    def __init__(self, open_vtd = False,vtdPort = 48190,sensorPort = 48195, plugins_path = 'data/lib/sendTrigger.so',test = False,args = None):
        ...
        if self.open_vtd:
            ...
            # 强化学习模型动作空间中各个动作执行时间
            self.ACTIONS_DUR =  { 'LANE_LEFT': 100, 'IDLE': 0 , 'LANE_RIGHT': 100, 'FASTER': 10, 'SLOWER': 10,
                                "LEFT_1": 80, "LEFT_2": 80, "RIGHT_1": 80, "RIGHT_2": 80}
            self.action_marking = ''
            # 各个动作预留时间         
            self.keep_time = -1
            # 各个动作已用动作时间
    ...
    def exec_lat_act(self):
        ...
        elif self.ctrl_signal['lat'] == "LEFT_2"  and right_lane :
    ...
    def collision_warning(self):
        ...
        print("ADV---dis-safe_dis:",self.get_distance(self.gl.ego)-(safe_dis))

        if lat_request == True and self.get_distance(self.gl.ego) > safe_dis + 4.5:
            lat_warn = False
        else:
            lat_warn = True
