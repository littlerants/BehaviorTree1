...
class ADV_Manager:
    ...
    def get_dqn_state_new(self):
        ...
        obs = [self.gl.ego.pos_x, self.gl.ego.pos_y, self.gl.ego.vx, self.gl.ego.vy,\
               self.get_speed(self.gl.adv.vx,self.gl.adv.vy),2*np.pi -self.gl.ego.pos_h if self.gl.ego.pos_h > np.pi else self.gl.ego.pos_h, \
    ...
    def collision_warning(self):
        ...
        other_vecs = sorted(self.gl.objects, key=lambda x: math.sqrt(x.pos_x ** 2 + x.pos_y ** 2))
        # 建立对抗车车辆坐标系
        # for i in other_vecs:
            # i.show()
        # print("ADV--->>>>>>>>>>>>>>>>>>>>>>>>>>")
        ...
        # for i in range(1000):
            ...
            if self.gl.adv is not None:
                ...
                if self.keep_time > 0 and  self.keep_time_index + 1 == self.keep_time:
                    ...
                    # 获取模型输入
                    # state,ita_state,ped_state = self.get_dqn_state_new()
