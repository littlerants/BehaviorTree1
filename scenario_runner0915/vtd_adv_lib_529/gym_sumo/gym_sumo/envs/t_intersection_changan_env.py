...
class TInterSectionEnv(AbstractEnv):
    """
    ...
    def default_config(cls) -> dict:
        ...
        cfg.update({
            ...
            'make_road': False,
            'map_file_path': '/home/sx/wjc/wjc/datasets/ca_vtd_train_data/dingzilukou/T_Junction_Two_Way_Four_Lane-0001.xodr',
            'ACTIONS': {0: 'LANE_LEFT', 1: 'IDLE', 2: 'LANE_RIGHT', 3: 'FASTER', 4: 'SLOWER',
                        5: "LEFT_1", 6: "LEFT_2", 7: "RIGHT_1", 8: "RIGHT_2"},
            'AtomicScene': 'lat_mode',  # lon lat
            'vehicles_num': 20,
        ...
        if self.config['action']['type'] == 'DiscreteMetaAction':
            if self.config['AtomicScene'] == 'lon_mode':
                ...
                if last_action is not None:
                    if (self.config['ACTIONS'][int(action)] != 'IDLE' and self.config['ACTIONS'][int(last_action)] != 'IDLE' and
                ...
                if abs(d1) > 5:
                    ...
                    if d2 >= (3 + d1):
                        ...
                        reward += -(1 - math.exp(- abs(d1-d2 + 3) * 0.9))
    ...
    def make_scenario(self, ego_road_info, test_av_road_info, ego_obj_road_info, test_av_obj_road_info):
        ...
        ego_obj_lane_index = self.road.network.get_lane_index_by_road(ego_obj_road_info[0], ego_obj_road_info[1], ego_obj_road_info[2])
        test_av_obj_lane_index = self.road.network.get_lane_index_by_road(test_av_obj_road_info[0], test_av_obj_road_info[1],
        ...
        self.vehicle = ego_vehicle_type.make_on_lane(self.road, 'ego', (ego_lane_index[0], ego_lane_index[1], 0), ego_lon,
                                               speed=ego_speed)
    ...
    def _make_vehicles(self) -> None:
        ...
        elif self.config['AtomicScene'] == 'ita_mode_bv_left_turn':

            scenarios = {
                'stop_left_turn_1': {'ego_road_info': (130, 0, -1), 'test_av_road_info': (103, 0, -1), 'ego_obj_road_info': (130, 0, -1), 'test_av_obj_road_info': (103, 0, -1)},
                'stop_left_turn_1_1': {'ego_road_info': (130, 0, -2), 'test_av_road_info': (103, 0, -1), 'ego_obj_road_info': (130, 0, -2), 'test_av_obj_road_info': (103, 0, -1)},
                'stop_left_turn_1_2': {'ego_road_info': (133, 0, -1), 'test_av_road_info': (103, 0, -1), 'ego_obj_road_info': (133, 0, -1), 'test_av_obj_road_info': (103, 0, -1)},
                'stop_left_turn_1_3': {'ego_road_info': (115, 0, -1), 'test_av_road_info': (103, 0, -1), 'ego_obj_road_info': (115, 0, -1), 'test_av_obj_road_info': (103, 0, -1)},
                'stop_left_turn_2': {'ego_road_info': (103, 0, -1), 'test_av_road_info': (115, 0, -1), 'ego_obj_road_info': (103, 0, -1), 'test_av_obj_road_info': (115, 0, -1)},
                'stop_left_turn_2_1': {'ego_road_info': (130, 0, -1), 'test_av_road_info': (115, 0, -1), 'ego_obj_road_info': (130, 0, -1), 'test_av_obj_road_info': (115, 0, -1)},
                'stop_left_turn_2_2': {'ego_road_info': (130, 0, -2), 'test_av_road_info': (115, 0, -1), 'ego_obj_road_info': (130, 0, -2), 'test_av_obj_road_info': (115, 0, -1)},
            }
