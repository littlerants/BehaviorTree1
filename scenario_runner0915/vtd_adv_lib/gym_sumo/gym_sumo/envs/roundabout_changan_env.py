...
class RoundaboutEnv(AbstractEnv):
    ...
    def default_config(cls) -> dict:
        ...
        cfg.update({
            ...
            'make_road': False,
            'map_file_path': '/home/sx/wjc/wjc/datasets/ca_vtd_train_data/3round/Roundabout_with_3_Three_Way_exits-0001.xodr',
    ...
    def make_scenario(self, ego_road_info, test_av_road_info, ego_obj_road_info, test_av_obj_road_info):
        ...
        base_speed = random.choice(range(15))
        ego_speed, bv_speed = random.choice(range(max(base_speed-5, 0), min(base_speed+5, 15))), random.choice(range(max(base_speed-5, 0), min(base_speed+5, 15)))
        if self.config['AtomicScene'] in ['lon_mode', 'lat_mode']:
            ...
            _, intersection_center = check_lane_intersection(ego_center, ego_lane_width, bv_center, bv_lane_width)
        ...
        self.vehicle = ego_vehicle_type.make_on_lane(self.road, 'ego', (ego_lane_index[0], ego_lane_index[1], 0), ego_lon, speed=ego_speed)
        ...
        if self.config['obs_v1']:
            ...
            #         speed=random.choice(range(max(base_speed-5, 0), min(base_speed+5, 15)))
            #     )
    ...
    def _make_vehicles(self) -> None:
        ...
        if self.config['AtomicScene'] == 'lon_mode':
            ...
            if road_info[0] != 7:
                scenarios = {
                    ...
                    'scene_2': {
                        ...
                        'test_av_road_info': (road_info[0], 0, -1),
                        'ego_obj_road_info': (road_info[0], 0, -2),
                        'test_av_obj_road_info': (road_info[0], 0, -2)
                    },
                    'scene_3': {
                        'ego_road_info': (road_info[0], 0, -3),
                ...
                scenarios = {
                    'scene_1': {
                        'ego_road_info': (road_info[0], 0, -3),
                        'test_av_road_info': (road_info[0], 0, -2),
                        'ego_obj_road_info': (road_info[0], 0, -3),
                        'test_av_obj_road_info': (road_info[0], 0, -3)
                    },
                    'scene_2': {
                        'ego_road_info': (road_info[0], 0, -2),
                        'test_av_road_info': (road_info[0], 0, -1),
            ...
            road_infos = [(2, 28), (4, 15), (6, 22)]
            road_info = random.choice(road_infos)
            scenarios = {
                'scene_1': {
                    'ego_road_info': (road_info[0], 0, -3),
                    'test_av_road_info': (road_info[1], 0, -1),
                    'ego_obj_road_info': (road_info[0]+1, 0, -3),
                    'test_av_obj_road_info': (road_info[0]+1, 0, -3)
                },
                'scene_2': {
                    'ego_road_info': (road_info[1], 0, -1),
                    'test_av_road_info': (road_info[0], 0, -3),
                    'ego_obj_road_info': (road_info[0]+1, 0, -3),
                    'test_av_obj_road_info': (road_info[0]+1, 0, -3)
                },
