...
class ADV_Manager:
    ...
    def __init__(self, open_vtd = False,vtdPort = 48190,sensorPort = 48195, plugins_path = 'data/lib/sendTrigger.so',test = False,args = None):
        ...
        if self.open_vtd:
            ...
            # 强化学习模型动作空间中各个动作执行时间
            self.ACTIONS_DUR =  { 'LANE_LEFT': 60, 'IDLE': 0 , 'LANE_RIGHT': 60, 'FASTER': 10, 'SLOWER': 10,
                                "LEFT_1": 30, "LEFT_2":30, "RIGHT_1": 30, "RIGHT_2": 30,"BLANK":20}
            self.action_marking = ''
            # 各个动作预留时间         
            self.keep_time = -1
            # 各个动作已用动作时间
    ...
    def exec_lat_act(self):
        ...
        if left_lane or right_lane:
            lane_off_set = 1.2 + gain
        else:
            lane_off_set = 1.2
        # lane_off_set = 0.5
        # lane_off_set -= gain
        print(">>>>>>>>>left_lane and right_lane>>>>>>>>>>>:",left_lane,right_lane)
        if self.ctrl_signal['lat']  == "LANE_LEFT" and left_lane :
            self.lane_left_act()

        elif self.ctrl_signal['lat'] == "LEFT_2" and left_lane :
            self.left_2_act(lane_off_set)

        elif self.ctrl_signal['lat'] == "LANE_RIGHT" and right_lane :
            self.lane_right_act()

        elif self.ctrl_signal['lat'] == "RIGHT_2" and right_lane:
    ...
    def stop_lat_act(self,flag = True):
        if self.action_marking == "BLANK":
            ...
            self.keep_time = self.ACTIONS_DUR["BLANK"]
            self.keep_time_index = -1
    ...
    def contrl_adv(self,lat_warn):
        ...
        if    (self.gl.ego.vx >  0 and    ego_speed*0.5 > adv_speed) or lat_warn:
            print("lat_warn:",lat_warn)
        ...
        if lat_warn or (self.gl.ego.pos_x > 0 and self.gl.adv.lane_id != self.gl.ego.lane_id):
            # print("lat warn :", lat_warn)
            print("stop lat act !!!")
            if self.action_marking in ["LANE_LEFT", "LANE_RIGHT"]:
                print("lane change action can not stop!!!")
            else:
                print(">>>>>>stop lat act!!!>>>>>>>>")
                self.stop_lat_act()
                return -1
        ...
        #     return -1
            ...
            if self.action_marking in ['LANE_RIGHT','LANE_LEFT'] and self.gl.adv.lane_id == self.gl.pre_adv_lane_id and self.keep_time < 120 :
                self.keep_time += 10
            else:
                self.stop_lat_act()
            return -1
    ...
    def slow_following_check(self, lat_warn):
        ...
        # 如果对抗车速度小于2m/s，则抑制掉横向对抗动作，此处不影响慢车换道逻辑，result会在满足慢车换道逻辑后重新刷新
        if adv_speed < 2:
            print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>ADV speed < 2!!!!!!!!!!!!")
            result =  'IDLE'

        if  adv_speed < 1.5 and self.gl.bake_vec_to_compete is not None:
    ...
    def space(self) -> spaces.Space:
        return spaces.Box(shape=(1, self.args.state_dim_1), low=-np.inf, high=np.inf,
                          dtype=np.float32)
    ...
    def get_dqn_state_new(self):
        ...
        left_lane  = 1 if abs(self.gl.adv.leftLaneId) < 30 else 0
        right_lane = 1 if abs(self.gl.adv.rightLaneId) < 30 else 0
        adv_ahead_has_end_point = False
        distance_ratio = 0
        if adv_ahead_has_end_point:
            adv_current_lon, _ = self.gl.ego.lane.local_coordinates(np.array([self.gl.adv.pos_x, self.gl.adv.pos_y]))
            distance_ratio = (30 - (self.gl.adv.lane.length - adv_current_lon)) / self.gl.ego.lane.length
        # 主车相对于对抗车的  相对位置x，y，相对速度 vx  vy  ，  对抗车的绝对速度      主车相对于对抗车的相对航向角，
        # 左变道，右变道，
        obs = [self.gl.ego.pos_x, self.gl.ego.pos_y, self.gl.ego.vx, self.gl.ego.vy,\
               self.get_speed(self.gl.adv.vx,self.gl.adv.vy), self.gl.ego.pos_h - 2 * np.pi if self.gl.ego.pos_h > np.pi else self.gl.ego.pos_h,\
    ...
    def sample_actions_new(self,state):
        with torch.no_grad():
            action = self.model_manager.model.value_net( state )
        ...
        while 1:
            ...
            if self.gl.adv is not None:
                ...
                if self.keep_time > 0 and  self.keep_time_index + 1 == self.keep_time:
                    ...
                    print("ADV--->>>>>>>>>>>>>>>>>>>>model_ctrl>>>>>>>>>>>>>>>>>>>>:", self.ctrl_signal)
