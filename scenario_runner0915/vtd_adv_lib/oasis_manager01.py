...
class PARSER():
    def __init__(self):
        self.output_path = "/home/zjx/work/simulate_VTD_ADV_traffic/data/model/"
        self.load_model_id = "intersection_lon"
        self.load_model_id_lat = "intersection_lat"
        self.load_model_id_ita = "intersection_ita"
        self.load_model_id_ped = "intersection_ped"
        self.state_dim = 36
        self.action_dim = 3
        self.state_dim_lat = 36
        self.action_dim_lat = 7
        self.state_dim_ita = 18
        self.action_dim_ita = 3
        self.state_dim_ped = 18
        self.action_dim_ped = 3
...
def make_args():
    ...
    # adv_mode_path = os.path.join(os.getenv('ADVPATH'), 'data/adv_model')
    adv_mode_path = "/home/zjx/work/simulate_VTD_ADV_traffic/data/model/"
    print('adv_mode_path:', adv_mode_path)
    config = ['ramp', 'exp_6',
              ...
              adv_mode_path
              # '/home/sda/upload/VTD.2021/VTD.2021.3/Develop/ca/changan_adv_model_1_10/data/model/model/'
              # '/home/sda/upload/VTD.2021/VTD.2021.3/Develop/ca/changan_adv_model_1_10/data/model/model/ramp/3_1'
              # '/home/sda/upload/VTD.2021/VTD.2021.3/Develop/ca/changan_adv_model_1_10/data/model/model/pedestrain'
              ...
              # '/home/sda/upload/VTD.2021/VTD.2021.3/Develop/ca/changan_adv_model_1_10/data/model/model/intersection/'
    ...
    args.add_argument('--state_dim_ped', default=18, type=int)
    args.add_argument('--action_dim_ped', default=3, type=int)

    args.add_argument('--env_config', default=config[2], type=dict)  # lat_mode lon_mode
    args.add_argument('--output_path', default=adv_mode_path,type=str)  # lat_mode lon_mode
    args.add_argument('--vehicles_count', default=6)  # lat_mode lon_mode
    args.add_argument('--adv_type', default=1, type=int)  # scenario type
    # model = 3
    # adv_type = args.parse_args().adv_type
    adv_type =1
    print("adv_type:", adv_type)
    if adv_type == 1:
        # intersection
        args.add_argument('--load-model-id', default="intersection_lon", type=str)  # 454  374
        args.add_argument('--load-model-id-lat', default="intersection_lat", type=str)  # exp4 400 407 exp5 417 455  370
        args.add_argument('--load-model-id-ita', default="intersection_ita", type=str)  # exp4 400 407 exp5 417 455  370
        # args.add_argument('--load-model-id-ped', default="ped_lon", type=str)  # 348
    if adv_type == 2:
        # # ramp 454  374
        # # 3-1   497  341
        args.add_argument('--load-model-id', default="ramp_lon", type=str)  # 454  374
        args.add_argument('--load-model-id-lat', default="ramp_lat", type=str)  # exp4 400 407 exp5 417 455  370
        args.add_argument('--load-model-id-ita', default="ramp_ita", type=str)  # exp4 400 407 exp5 417 455  370
        args.add_argument('--load-model-id-ped', default="ped_lon", type=str)  # 348
    if adv_type == 3:
        # roundabout   lon: 363    lat:485  ita:434  ped 296
        args.add_argument('--load-model-id', default="round_lon", type=str)  # 454  374
        args.add_argument('--load-model-id-lat', default="round_lat", type=str)  # exp4 400 407 exp5 417 455  370
        args.add_argument('--load-model-id-ita', default="round_ita", type=str)  # exp4 400 407 exp5 417 455  370
        args.add_argument('--load-model-id-ped', default="ped_lon", type=str)  # 348
    if adv_type == 4:
        # lanechange
        args.add_argument('--load-model-id', default="lanechange_lon", type=str)  # 454  374
        args.add_argument('--load-model-id-lat', default="lanechange_lat", type=str)  # exp4 400 407 exp5 417 455  370
        args.add_argument('--load-model-id-ita', default="lanechange_ita",
                          type=str)  # exp4 400 407 exp5 417 455  370
        args.add_argument('--load-model-id-ped', default="ped_lon", type=str)  # 348
    if adv_type == 5:
        # pedestrain  348
        args.add_argument('--load-model-id', default="ramp_lon", type=str)  # 454  374
        args.add_argument('--load-model-id-lat', default="ped_lat", type=str)  # exp4 400 407 exp5 417 455  370
        args.add_argument('--load-model-id-ita', default="ped_ita", type=str)  # exp4 400 407 exp5 417 455  370
        args.add_argument('--load-model-id-ped', default="ped_lon", type=str)  # 348
    return args
...
class ADV_Manager(AtomicBehavior):
    ...
    def get_local_velocity(self, vehicle, velocity):
        """将全局坐标系下的坐标转到局部坐标系下
        ...
        """
        res = np.array(vehicle.get_transform().get_inverse_matrix()).dot(
            np.array([velocity.x, velocity.y, velocity.z, 1])
        )
        return carla.Vector3D(x=res[0],y=res[1],z=res[2])
    def get_local_acc(self, vehicle, acc):
        """将全局坐标系下的坐标转到局部坐标系下
    ...
    def create_objs(self):
        ...
        if self.gl.ego is not None:
            ...
            self.gl.ego.update(pos_x=self.ego_vehicle.get_location().x, pos_y=-self.ego_vehicle.get_location().y,
                               ...
                               acc_x=self.ego_vehicle.get_acceleration().x, hdg=ego_wp.transform.rotation.yaw, \
        ...
        for i in objects:
            ...
            # velocity = self.get_local_velocity(self.ego_vehicle,i.get_velocity())
            # acc = self.get_local_acc(self.ego_vehicle, i.get_acceleration())
                ...
                for j in self.gl.objects:
                    if i.id == j.id:
                        j.update(pos_x=location.x, pos_y=-location.y,
                                 pos_h=i.get_transform().rotation.yaw, hdg=i_wp.transform.rotation.yaw, \
    ...
    def parseRDBMessage(self):
        ...
        print("ADV---$$$$$$$$$$$$$$end$$$$$$$$$$$$$$$",len(self.gl.objects))
        ...
        if len(self.gl.objects) >0:
            ...
            for i in self.gl.objects:
                ...
                # 主车50米以内所有车辆
                if self.get_sqrt(i.pos_x, i.pos_y) < 100 and i.lane_id * self.gl.ego.lane_id > 0:
                    # 主车后30米以内所有车辆
    ...
    def trans_world_cood(self):
        ...
        if self.gl.adv is not None:
            ...
            self.gl.adv.trans_cood1(self.gl.ego, True, True,True,rotate=1)
        for i in self.gl.objects:
            i.trans_cood1(self.gl.ego, True,True,True,rotate=1)
