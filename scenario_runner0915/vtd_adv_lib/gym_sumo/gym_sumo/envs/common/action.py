...
from gym_sumo.vehicle.dynamics import BicycleVehicle
...
class ActionType(object):
    ...
    def vehicle_class(self) -> Callable:
        ...
        Must return a subclass of :py:class:`gym_sumo.vehicle.kinematics.Vehicle`.
        """
        raise NotImplementedError
    ...
    def act(self, action: Action) -> None:
        ...
        vehicle is an instance of the specified :py:class:`gym_sumo.envs.common.action.ActionType.vehicle_class`.
...
class MultiDiscreteMetaAction(ActionType):
    ACTION_MAP = [
        # ALL
        {    0: 'LANE_LEFT',    1: 'IDLE',    2: 'LANE_RIGHT',    3: 'FASTER',    4: 'SLOWER'},
        # no left turn
        {    0: 'IDLE',    1: 'LANE_RIGHT',    2: 'FASTER',    3: 'SLOWER'},
        # no right turn
        {0: 'LANE_LEFT', 1: 'IDLE', 2: 'FASTER', 3: 'SLOWER'},
        # unable to turn
        {    0: 'IDLE',    1: 'FASTER',    2: 'SLOWER'},
    ]
    ...
    def space(self) -> spaces.Tuple:
        sp = []
        for v in self.ACTION_MAP:
            sp.append(spaces.Discrete(len(v)))
        return spaces.Tuple(sp)

    @property
    def vehicle_class(self) -> Callable:
        return functools.partial(MDPVehicle, target_speeds=self.target_speeds)

    def act(self, action: Union[int, np.ndarray, List, ]) -> None:
        self.controlled_vehicle.act(self.ACTION_MAP[action[0]][int(action[1])])
...
def action_factory(env: 'AbstractEnv', config: dict) -> ActionType:
    ...
    elif config["type"] == "MultiDiscreteMetaAction":
        return MultiDiscreteMetaAction(env, **config)
