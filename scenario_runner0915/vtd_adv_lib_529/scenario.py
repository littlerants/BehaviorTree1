        ...
        for node in root.iter("Scenario"):
            for node1 in node.iter():
                ...
                if node1.tag == 'Action':
                    ...
                    # if node1.attrib['Name'] == 'wallpoint':
                    if node1.attrib['Name'].find('wallpoint') != -1:
                        # print("node tag:",node1.tag)
                        # print("attrib:",node1.attrib)
                        for node2 in node1.iter():
                            if node2.tag == 'PosAbsolute':
                                # print("node tag:",node2.tag)
                                # print("attrib:",node2.attrib)
                                print("wallpoints position:")
                                print("x/y:", "( ", float(node2.attrib['X']), ",", float(node2.attrib['Y']),
                                      ")")
                                self.wallpoints.append([float(node2.attrib['X']) ,float(node2.attrib['Y']) ])
                    if node1.attrib['Name'].find('adv_enable') != -1:
                        # print("node tag:",node1.tag)
                        # print("attrib:",node1.attrib)
                        for node2 in node1.iter():
                            if node2.tag == 'PosAbsolute':
                                # print("node tag:",node2.tag)
                                # print("attrib:",node2.attrib)
                                print("adv_enable position:")
                                print("x/y:", "( ", float(node2.attrib['X']), ",", float(node2.attrib['Y']),
                                      ")")
                                self.adv_enable.append([float(node2.attrib['X']) ,float(node2.attrib['Y']) ])
# 场景管理类，
class SCENARIO():
