...
class ConstantVelocityAgent(BasicAgent):
    """
    ConstantVelocityAgent implements an agent that navigates
    the scene at a fixed velocity.
    This agent will fail if asked to perform turns that are impossible are
    the desired speed.
    This includes lane changes. When a collision is detected, the constant
    velocity will stop,
    ...
    def __init__(
        ...
        Initialization the agent parameters, the local and the global planner.
            ...
            :param opt_dict: dictionary in case some of its parameters
            want to be changed.
                This also applies to parameters related to the LocalPlanner.
            :param map_inst: carla.Map instance to avoid the expensive
            call of getting it.
            :param grp_inst: GlobalRoutePlanner instance to avoid the
            expensive call of getting it.
        """
        super().__init__(
            vehicle,
            target_speed,
        ...
        # Whether or not to use the BasicAgent behavior
        # when the constant velocity is down
        ...
        # Time after collision before the constant velocity
    ...
    def stop_constant_velocity(self):
        ...
        self._constant_velocity_stop_time = (
            self._world.get_snapshot().timestamp.elapsed_seconds
        )
    ...
    def run_step(self):
        ...
        if affected_by_vehicle:
            ...
            if vehicle_velocity.length() == 0:
                ...
                hazard_speed = (
                    vehicle_velocity.dot(adversary.get_velocity())
                    / vehicle_velocity.length()
                )
        ...
        # still useful to apply it so that the vehicle isn't moving
        # with static wheels
