...
from gym_sumo.road.road import Road, LaneIndex, Route
...
class ControlledVehicle(Vehicle):
    ...
    def plan_route_to(self, destination: Tuple) -> "ControlledVehicle":
        """
        Plan a route to a destination in the road network

        :param destination: a node in the road network
        """
        if self.lane_index[:2] == destination:
            self.route = [self.lane_index[0], self.lane_index[1]]
            return self

        try:
            path = self.road.network.shortest_path((self.lane_index[0], self.lane_index[1]), destination)
        except KeyError:
            path = []
        if path:
            self.route = path
    ...
    def act(self, action: Union[dict, str] = None) -> None:
        ...
        if action == "FASTER":
            ...
            right_lane = self.road.network.get_right_lane(self.lane_index)
            self.target_lane_index = self.lane_index if right_lane is None else (right_lane[0], right_lane[1], 0)
        elif action == "LANE_LEFT":
            left_lane = self.road.network.get_left_lane(self.lane_index)
            self.target_lane_index = self.lane_index if left_lane is None else (left_lane[0], left_lane[1], 0)
        elif action =="IDLE":
            self.target_speed = self.speed
        elif action == "LEFT_1":
            long, lat = self.lane.local_coordinates(self.position)
            self.pos_offset = -(1 - lat)
        elif action == "RIGHT_1":
            long, lat = self.lane.local_coordinates(self.position)
            self.pos_offset = lat + 1
        elif action == "LEFT_2":
            long, lat = self.lane.local_coordinates(self.position)
            w = self.lane.width_at(long)
            self.pos_offset = -(w / 2 - 0.1 - lat)
        elif action == "RIGHT_2":
            long, lat = self.lane.local_coordinates(self.position)
            w = self.lane.width_at(long)
            self.pos_offset = lat + w / 2 - 0.1
    ...
    def follow_road(self) -> None:
        ...
        if self.road.network.get_lane(self.target_lane_index).after_end(self.position):
            ...
            if self.target_lane_index == self.lane_index and not self.get_name.startswith('ped'):
                # if self.route:
                #     self.plan_route_to((self.route[-2], self.route[-1]))

                self.target_lane_index = self.road.network.next_lane(self.lane_index,
                                                                     route=self.route,
                                                                     position=self.position,
                                                                     np_random=self.road.np_random,
                                                                     vehciel_length=self.LENGTH)
    ...
    def steering_control(self, target_lane_index: LaneIndex) -> float:
        ...
        lateral_speed_command = - self.KP_LATERAL * (lane_coords[1]) if (not hasattr(self, 'pos_offset') or self.pos_offset is None) \
            else - self.KP_LATERAL * self.pos_offset
...
class MDPVehicle(ControlledVehicle):
    ...
    def __init__(self,
        ...
        """
        super().__init__(road, name, position, heading, speed, target_lane_index, target_speed, route, lane_index)
